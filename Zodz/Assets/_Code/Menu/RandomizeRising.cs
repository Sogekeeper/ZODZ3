...
public class RandomizeRising : MonoBehaviour
{
	public UpdateRisingInfo[] lunarPanels;
	...
	private void Awake()
	{
		if (canDraftEqualsRaces)
		{
			for (int i = 0; i < lunarPanels.Length; i++)
			{
				...
				lunarPanels[i].raceToGetRisingInfo = random;
			}
		}
		else
		{
			int index = Random.Range(0, racesScript.lunarAndRisingRaces.Length);
			Race random = racesScript.lunarAndRisingRaces[index];
			lunarPanels[0].raceToGetRisingInfo = random;
			...
			lunarPanels[1].raceToGetRisingInfo = random2;
			...
			lunarPanels[2].raceToGetRisingInfo = random3;
