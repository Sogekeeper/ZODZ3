    ...
    void DrawIndicators()
    {
        foreach(Target target in targets)
        {
            ...
            else if(target.NeedArrowIndicator && !isTargetVisible)
            {
                ...
                if(target.RotateArrowIndicator)indicator.transform.rotation = Quaternion.Euler(0, 0, angle * Mathf.Rad2Deg); // Sets the rotation for the arrow indicator.
            }
            if(indicator)
            {
                indicator.SetImageColor(target.TargetColor);// Sets the image color of the indicator.
                if(target.TargetSprite) indicator.SetImageSprite(target.TargetSprite);
